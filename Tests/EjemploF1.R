...
g = ggplot(df) + 
  geom_point(aes(x=NGPSLongitude,y=NGPSLatitude,col=sign(gLat),size=abs(gLat)))
...
g = ggplot(subset(df,Lap==22)) + 
  geom_line(aes(x=sLap,y=vCar,colour=NGear))

print(g)

#We can also do statistical graphics - like a boxplot showing the distribution of speed values by gear
g = ggplot(df) + 
  geom_boxplot(aes(x=factor(NGear),y=vCar))

print(g)

#Footwork - brake and throttle pedal depression based on gear
g = ggplot(df) + 
  geom_jitter(aes(factor(NGear),rThrottlePedal),colour='darkgreen') + 
  geom_jitter(aes(factor(NGear),pBrakeF),colour='darkred')
...
g = ggplot(df) + 
  geom_jitter(aes(factor(NGear),gLong,col=pBrakeF)) + 
...
g = ggplot(df) + 
  geom_jitter(aes(factor(NGear),gLong,col=rThrottlePedal)) + 
  scale_colour_gradient(low='red',high='green')
print(g)
...
ggplot(df) + 
  geom_point(aes(x=nEngine,y=vCar,col=factor(NGear)))
